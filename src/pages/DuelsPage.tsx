...
import { RotateCcw, Plus, Loader2, Swords, Search } from "lucide-react";
...
export default function DuelsPage() {
  ...
  const allCharIds = Array.from(new Set(filteredDuels.flatMap(d => [d.characterAId, d.characterBId])));
  const { data: allChars = [] } = useCharactersByIds(allCharIds);
  const charMap = Object.fromEntries(allChars.map(c => [c.id, c]));

  const handleNext = () => {
    if (currentIndex + 1 >= filteredDuels.length) setCompleted(true);
  ...
  const duel = filteredDuels[currentIndex];
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-muted/20 pb-20 md:pb-0">
      ...
      <main className="flex-1 container mx-auto px-4 py-6 flex flex-col items-center">
        ...
        <div className="w-full max-w-5xl mb-4 flex gap-2 items-center">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input
              placeholder={t("ph_search_duels")}
              value={searchTerm}
              onChange={e => {
        ...
        {isLoading ? (
          ...
          <div className="text-center space-y-6 max-w-md mx-auto py-20">
            ...
            <p className="text-muted-foreground">
              {searchTerm ? "Aramanızla eşleşen düello bulunamadı." : "İlk düelloyu sen oluştur!"}
            </p>
            {user && !searchTerm && (
          ...
          <>
            <div className="w-full flex items-center justify-between max-w-5xl mb-4">
              <div className="text-sm font-mono text-muted-foreground tracking-widest uppercase">
                {`${t("lbl_duel")} ${currentIndex + 1} / ${filteredDuels.length}`}
              </div>
            </div>
            ...
            <div className="w-full max-w-5xl">
              ...
              {duel && (!charMap[duel.characterAId] || !charMap[duel.characterBId]) && (
                <div className="flex flex-col items-center gap-4 py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">Karakterler yükleniyor...</p>
                </div>
              )}
            </div>
