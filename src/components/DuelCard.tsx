...
export function DuelCard({ duel, charA, charB, onNext }: DuelCardProps) {
  ...
  const [votedFor, setVotedFor] = useState<"A" | "B" | null>(alreadyVoted ? "A" : null);
  ...
  const handleVote = async (choice: "A" | "B") => {
    if (votedFor || pending || showResults) return;
  ...
  const clickable = !votedFor && !pending && !showResults;
